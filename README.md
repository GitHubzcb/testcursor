...
内置识别器采用**模板匹配为主 + 结构特征为辅**的混合策略：

### 模板匹配（主要方法，更精确）

1. **模板生成**：程序启动时使用 OpenCV 内置字体（5 种字体 × 3 种尺寸 × 3 种粗细 = 45 套模板集）自动渲染 0-9 的标准模板
2. **归一化**：将待识别字符和模板都归一化到 20×32 像素
3. **IoU 相似度匹配**：计算待识别字符与每个模板的 Intersection-over-Union（交并比），选择最高分数的数字
4. **置信度阈值**：IoU > 0.35 才接受匹配结果

优势：
- 对已知字体族的数字识别精度接近 100%
- 逻辑简单，不存在复杂的阈值和规则边界问题
- 多套模板覆盖各种字体风格

### 结构特征分类（后备方法）

当模板匹配置信度不足时，自动切换到结构特征分析：
- 孔洞数量和位置判断 0/4/6/8/9
- 宽高比判断 1
- 像素分布判断 2/3/5/7
...
4. **小数点识别**：通过与中位字符高度的比值自动检测
5. **负号识别**：通过宽高比（宽>>高）和像素密度判断

## 注意事项

- 本工具假设色条位于图像右侧（右 30% 范围内搜索），且颜色从上（高温）到下（低温）渐变
- 色条检测基于**颜色变化方向一致性**（连续颜色增量的点积），可正确区分色条与主图像中的热梯度
- 支持极窄色条（最小 3 像素宽）和紧贴图像边缘的色条
- 温度标注文字应位于色条顶部和底部附近
- 如果自动检测失败，使用 `--bar-x1/x2/y1/y2` 手动指定色条区域
